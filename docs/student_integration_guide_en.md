...
> **Visualizer dependency**: `make visualize` runs `scripts/run_openevolve_visualizer.py`, which expects a one-time shallow clone at `third_party/openevolve` (gitignored):
>
> ```bash
> git clone --depth 1 https://github.com/algorithmicsuperintelligence/openevolve.git third_party/openevolve
> ```
>
> Without that folder you will get an error with instructions. Without `make`, use: `uv run python scripts/run_openevolve_visualizer.py --path config/openevolve_output`
